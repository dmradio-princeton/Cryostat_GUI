...
class StartWindow(QtWidgets.QMainWindow):
    ...
    def create_cooldown_folder(self):
        t_start_eastern = time.time() - (60.0 * 60.0 * 4.0) # convert from UTC to Eastern time (-4hrs)
...
class MyTableWidget(QtWidgets.QTabWidget):
    ...
    def get_resplate_temperature(self):
        ...
        if MeasureMagCooling.isChecked():
            self.mag_temps = [290,275,250,225,200,175,150,125,90,75,50,39,30,20,10,4,3,2,1]
    ...
    def determine_temperature_cooling(self):
        ...
        if self.resplate_temp.reading > 0:
    ...
    def measure_Bfield(self):
        ...
        print('B-field measurement at T={:.0f}K'.format(temp))
        NAME = os.path.join(os.path.join(folder,'Bfield'),'Cold_measurement_X500AMP_1_' + '{:.0f}'.format(temp) + 'K')
