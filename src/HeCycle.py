...
class HeCycle(QMainWindow):
    ...
    def _widgets(self):
        ...
        self.led_heater = {}
        self.led_heater_label = {}
        self.led_heat_switch = {}
        self.led_heat_switch_label = {}
        ...
        for port in self.ports_nb:
            ...
            self.led_heater_label[port] = QtWidgets.QLabel("Heater (On/Off)")
            self.led_heater[port] = LedIndicator(self)
            self.led_heater[port].setDisabled(True)  # Make the led non clickable

            self.led_heat_switch_label[port] = QtWidgets.QLabel("HS (On/Off)")
    ...
    def _layout(self):
        ...
        for port in self.ports_nb:
            ...
            gbox_port[port].addWidget(groupBox_wait[port], 0, 0, 2, 1)
            gbox_port[port].addWidget(groupBox_pid[port], 0, 1, 2, 1)
            gbox_port[port].addWidget(groupBox_heat[port], 0, 2, 2, 1)

            gbox_port[port].addWidget(self.led_heater[port], 0, 3)
            gbox_port[port].addWidget(self.led_heater_label[port], 0, 4)

            gbox_port[port].addWidget(self.led_heat_switch[port], 1, 3)
            gbox_port[port].addWidget(self.led_heat_switch_label[port], 1, 4)

            gbox_port[port].addWidget(self.run_cycle_button[port], 2, 0, 1, 3)
...
class Runnable(QRunnable):
    def __init__(self, dev_nb, dev_sens, port, sp, p, i, waiting_time, heating_time, button, waiting_timer, heating_timer, pid_settings_line, led_heater, led_heat_switch):
        super().__init__()
    ...
    def run(self):
        ...
        for i in range(self.waiting_time,0,-1):
            sys.stdout.write("\r")
            sys.stdout.write("Turn on heater in {}".format(datetime.timedelta(seconds=i)))
            self.waiting_timer.setText("Turn on heater in {}".format(datetime.timedelta(seconds=i)))
            sys.stdout.flush()
            time.sleep(1)
        ...
        self.pid_settings_line.setText(f'Set I=100')
        self.data_nb[self.port].handle_I('D1', 100)
